...
def subimage_iterator(img, width, height, ax, step_size, callback):
    ...
    green_square = np.zeros(img.shape)
    green_square[0:width, 0, 1] = 1
    green_square[0:width, height, 1] = 1
    green_square[0, 0:height, 1] = 1
    green_square[width, 0:height, 1] = 1
    ...
    for i in range(0, N, step_size):
        for j in range(0, M, step_size):
            square = np.roll(img, (i, j), axis=(0, 1))[:height, :width, :]
            ax.imshow(img + np.roll(green_square, (i, j), axis=(0, 1)))
            yield square, i, j
    ###
...
def highlight_circle(results, img, height, width, figure, ax, fig, nth=4):
    ...
    square[0, 0:width, 1] = 1
    square[height, 0:width, 1] = 1
    square[0:height, 0, 1] = 1
    square[0:height, width, 1] = 1

    ax.imshow(img + np.roll(square, (results['y'], results['x']), axis=(1, 0)))
    ###
